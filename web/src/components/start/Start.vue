<template>
    <div id="Start" v-show="show">
        ...
        <div class="base-div">
            ...
            <el-select class="select" :value="contractVersion" @click.native="getCompilers" @change="compilerChanged" placeholder="Please select Compiler Version" size="mini">
                <el-option-group>
                    <el-option v-for="(item, index) in compilers" :key="index" :label="item" :value="item"></el-option>
                </el-option-group>
                <el-option-group v-if="!compilersIsAll">
                    <el-option label="install other" value="install"></el-option>
                </el-option-group>
            </el-select>
        </div>
...
export default {
    ...
    methods: {
        ...
        getCompilers: function() {
            window.vscode.installedCompilers().then(msg => {
                this.compilers = msg.data.installed;
                this.compilersIsAll = !(msg.data.uninstalled.length);
            });
        },
        compilerChanged: function(res) {
            if (res === 'install') {
                window.vscode.installCompiler();
            } else {
                this.contractVersion = res;
            }
        },
        start: function() {
    ...
    mounted: function() {
        ...
        window.vscode.installedCompilers().then(msg => {
            this.compilers = msg.data.installed;
            this.contractVersion = this.compilers && this.compilers[0];
        });
