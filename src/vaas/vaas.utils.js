...
class SolcPickItem {
    constructor(solcPackage = pkg.solcPackages[0]) {
        this.solcPackage = solcPackage;
        this.label = `solc ${this.version}`;
        // this.description = '';
        this.detail = `Ethereum Smart Contract Compiler.`;
        this.picked = false;
        this.alwaysShow = false;
    }
    get version() {
        return this.solcPackage && this.solcPackage.version;
    }
    get isDownloading() {
        return this.solcPackage && this.solcPackage.isDownloading;
    }
    get description() {
        return this.isDownloading ? `(downloading...)` : '';
    }
}
const solcPickItems = pkg.solcPackages.map(item => {
    return new SolcPickItem(item);
});
let extName = '';
const installVaas = (name = '') => {
    name && (extName = name);
    return new Promise((resolve, reject) => {
        const solcItems = solcPickItems.filter(item => {
            return item.solcPackage.isPending;
        });
        vscode.window.showQuickPick(solcItems, {
            onDidSelectItem: (item) => {},
            canPickMany: true,
            placeHolder: (extName ? `(${extName}) ` : '') + `Please select compilers you need to install`,
            ignoreFocusOut: false
        }).then(data => {
            if (data && data.length) {
                pkg.activate(data.map(item => {
