...
const solcBaseUrl_github = 'https://github.com/BeosinBeosin/Beosin-VaaS-ETH-Extension/raw/solc';
const solcBaseUrl_aliyun = 'https://solc.oss-cn-zhangjiakou.aliyuncs.com/eth';
const solcUnzipPath = path.join(vaasPackage.executeDir, 'solc', 'eth', platformStr);
fs.existsSync(vaasPackage.executeDir) && !fs.existsSync(solcUnzipPath) && mkdirP(solcUnzipPath);
class SolcPackage extends Package {
    constructor(version = '') {
        super();
        this.name = `solc-${version}`;
        this.url_github = `${solcBaseUrl_github}/${platformStr}/${version}.zip`;
        this.url_aliyun = `${solcBaseUrl_aliyun}/${platformStr}/${version}.zip`;
        this.url = this.url_aliyun;
        this.version = version;
        this.unzipPath = solcUnzipPath;
        this.unzipedPath = path.join(this.unzipPath, this.version);
        this.packagePath = path.join(Package.packageDir, `${this.name}.zip`);
    }
}
const solcPackages = (function() {
    // ETH `0.4.12-0.4.25 0.5.0-0.5.11`
    const arr = [];
    for (let i = 11; i >= 0; i--) {
        arr.push(new SolcPackage('0.5.' + i))
    }
    for (let i = 25; i >= 12; i--) {
        arr.push(new SolcPackage('0.4.' + i))
    }
    return arr;
})();
